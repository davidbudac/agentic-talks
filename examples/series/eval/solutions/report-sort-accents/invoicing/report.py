...
from .customers import display_name, normalise_name, sort_key
...
def summarise(rows, currency: str = None) -> list:
    ...
    ordered = sorted(totals, key=lambda key: (sort_key(key[0]), key[1]))
    return [totals[key] for key in ordered]
