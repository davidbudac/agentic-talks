...
from .customers import display_name, matches, normalise_name
...
def summarise(rows, currency: str = None, customer: str = None) -> list:
    ...
    for row in rows:
        ...
        if wanted and code != wanted:
            continue
        if customer and not matches(row["customer"], customer):
            continue
...
def build_report(rows, currency: str = None, customer: str = None) -> str:
    """Summarise and render in one call; what the CLI prints."""
    return render_table(summarise(rows, currency=currency, customer=customer))
