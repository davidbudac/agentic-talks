...
MAPPING = [
    ...
    # Talk 07 became a 21-slide evals and traces talk on 2026-09-24. The old
    # local-model (s28) and /voice (s33) fallbacks left the deck: see
    # workshops/evals/talk07-routing-archive.html (the workshop's Lab 6) and
    # talk07-habits-archive.html, and the record.sh slots ws-lab6-local and habits-voice.
    dict(talk="07", deck="measuring-what-works.html", slide=3, title="Why vibes don't scale",
    ...
    dict(talk="07", deck="measuring-what-works.html", slide=4, title="Start with a task-completion suite",
         illustrative="6 tasks from last month, 4/6 pass", runner="talk07.sh", field="cells[*].{task,passes,k}", kind="auto", fmt=t07_s5),
    dict(talk="07", deck="measuring-what-works.html", slide="12-13", title="Live: score two variants / Fallback: the recorded eval run",
         illustrative="task 1-4 x variant A/B '…'", runner="talk07.sh + record.sh 07-s13", field="cells, variants", kind="auto", fmt=t07_variants,
         note="The per-task sheet is in results/talk07/<ts>/summary.md; the screenshot slot needs record.sh 07-s13."),
    dict(talk="07", deck="measuring-what-works.html", slide=14, title="Evals tell you which model to use",
         illustrative="A $0.10/run, 3/10 accepted, $0.33/accepted; B $0.25/run, 9/10, $0.28", runner="talk07.sh VARIANT=models",
         field="variants.*.{pass_rate,cost_per_pass_usd}", kind="partial", fmt=t07_variants,
         note="Two models on our six tasks give pass counts and cost per pass (= cost per accepted task); redraw the bars by hand."),
    dict(talk="07", deck="measuring-what-works.html", slide=15, title="A trace is the whole transcript",
         illustrative="t1-t6 auth.ts session", runner="talk07.sh", field="example_trace.steps", kind="auto", fmt=t07_s8),
    dict(talk="07", deck="measuring-what-works.html", slide=16, title="Read traces for wasted turns",
    ...
    dict(talk="07", deck="measuring-what-works.html", slide=18, title="Anatomy of an agent trace",
