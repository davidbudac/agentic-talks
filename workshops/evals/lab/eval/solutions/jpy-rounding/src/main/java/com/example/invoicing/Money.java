...
public final class Money {
    ...
    public static String format(BigDecimal amount, String currency) {
        ...
        return String.format(Locale.ROOT, "%,." + exponent(code) + "f %s", round(amount, code), code);
    }
    ...
    public static List<BigDecimal> allocate(BigDecimal total, int parts, String currency) {
        ...
        int places = exponent(currency);
        BigInteger units = round(total, currency).movePointRight(places).toBigIntegerExact();
        ...
        for (int i = 0; i < parts; i++) {
            ...
            shares.add(new BigDecimal(share, places));
        }
        return shares;
    }
}
