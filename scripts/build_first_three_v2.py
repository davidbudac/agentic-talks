...
TOOLBOX = [
 ...
 slide('Document work has two different needs', panels((ill_sources(), 'Understand the sources', 'Gemini Notebook: explore supplied material and listen to an Audio Overview.'), (ill_present(), 'Present the material', 'Gamma or Canva: turn an outline into a presentation you can edit.')), 'A sourced summary and a designed deck are different outputs. Check where a claim came from before you spend time on its layout. An Audio Overview can contain errors, so use it to get oriented and then go back to the source.', '17, 24, 35, 37', 'Documents'),
 ...
 slide('Our loop animation is rendered from code', video('agent-loop-dark', 'An existing explainer from this repository · built with Remotion'), 'Play it. This is a real artifact from this repository: Remotion turns code into video, so the layout stays exactly as written. It did not come from a new vendor session. Narration is a separate category, with text-to-speech tools such as ElevenLabs: check the pronunciation and get permission before cloning anyone’s voice.', '30–32 → actual repository artifact', 'Media', dark=True),
...
ENGINEERING = [
 ...
 slide('Inspect the tests and the file they produce', tests_and_artifact(), 'Prepared result from after/: three invoice tests and two export tests. We generated the CSV with the same code and parsed it during validation. It is neither a benchmark nor a recorded agent run.', '23–25 → checked evidence', 'Check the evidence', foot='Prepared reference result · five tests'),
 ...
 slide('Go deeper where your work needs it', deck_cards(), 'Take questions. The later decks have not been revised: they keep their own dates, some are still in progress, and nobody has rechecked their product claims for this edition.', 'new series handoff', 'Questions'),
