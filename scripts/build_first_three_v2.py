...
def accepted_chart():
    ...
    chart = (m('Model + tool cost', '$6', '$10', 48, 80, '<small>headline</small>') + m('Accepted tasks, out of 10', '6', '10', 48, 80)
             + m('Cost per accepted task', '$1', '$1', 80, 80, '<small class="hot">equal</small>') + m('Review time, total', '40 min', '15 min', 80, 30))
    return f'''<div class="v-acc"><div role="img" aria-label="Illustrative trial. Setup A: $6 cost, 6 of 10 accepted, $1 per accepted task, 40 minutes review. Setup B: $10, 10 of 10, $1 per accepted task, 15 minutes review.">
<div class="legend"><b>Illustrative trial</b><span><i class="A"></i>Setup A</span><span><i class="B"></i>Setup B</span></div>{chart}</div>
<div class="call"><span class="k">Cost per accepted task</span><div class="big">$1 <span class="eq">=</span> $1</div><div class="hr"></div>
<span class="k">Review time with Setup B</span><div class="big o">−25<small>min</small></div><p>Same cost per accepted task — and B accepts 10 of 10, A only 6.</p></div></div>'''
...
<text x="486" y="196" text-anchor="middle" class="t-acc sm">tools</text>'''
    return panels((svg(700, 280, chat, 'Chat: you ask and receive an answer, then carry the work into your files yourself.'), 'Chat', 'You ask for help, then carry the work into your files.'),
                  (svg(700, 280, agent, 'Agent: it loops through tools and results, acting on your files.'), 'Agent', 'The system can use tools, inspect results and continue the task.'), cls='wide')
...
def context_supply():
    ...
    return (p('The assistant needs the sources, constraints and examples relevant to this task.') +
            ...
            callout('Give it enough to work with', 'A clear brief and the right files are usually more useful than a longer prompt.') + '</div>')
...
{sicon('lock', 760, 44, 44, 'lk')}{sicon('lock', 760, 264, 44, 'lk')}'''
    ...
    return split(p('Read a folder and send an email are different permissions.') + svg(800, 400, art, 'One connection can carry two different permissions: read a folder, send an email.'),
...
<circle cx="1540" cy="165" r="78" class="ok-dot"/><path d="M1504 167 L1530 193 L1578 139" class="ok-tick lg"/>'''
    steps = ''.join(f'<div><h3>{t}</h3><p>{d}</p></div>' for t, d in [('Summarise', 'Record decisions, files and remaining work.'), ('Restart', 'Open a fresh task session when useful.'), ('Verify', 'Check that the handoff kept the essentials.')])
...
<div class="neq" aria-hidden="true">≠</div><div class="th th-b"><div class="evs">{ev}</div><h3>Evidence of the result</h3><p>The changed file, command output and checks matter more.</p></div></div>'''
...
def review_list():
    return rows(('diff', 'Diff', 'Does the change match the requested behaviour?'), ('terminal', 'Checks', 'Were the tests actually run and preserved?'), ('target', 'Scope', 'Did anything unrelated change?'), cls='checklist')
...
def tests_and_artifact():
    term = window('prepared reference result', '<span class="p">$</span> python3 -m unittest -v\n\n<span class="dim">Ran 5 tests</span>\n<span class="okbig">OK</span>', 'term', 'v-okterm')
    rs = [('INV-001', 'North, Ltd', '121.00'), ('INV-002', 'Studio "A"', '0.61'), ('INV-003', 'Line↵break', '100.00')]
    tb = ''.join(f'<tr><td>{a}</td><td>{esc(b)}</td><td>{c}</td></tr>' for a, b, c in rs)
    art = f'''<div class="v-art"><div class="ah">{icon('sheet')}<span>invoices.csv</span></div><table><thead><tr><th>invoice_id</th><th>customer <b>names</b></th><th>gross <b>totals</b></th></tr></thead><tbody>{tb}</tbody></table><p class="cap">columns · names · totals</p></div>'''
    return f'<div class="v-ta">{term}{art}</div>' + p('Then open the CSV and compare its columns, names and totals with the contract.', 'sm')
...
def deck_cards():
    items = [('04', 'Subagents &amp; prompt caching', 'subagents-prompt-caching.html', ''), ('05', 'Cost &amp; context', 'cost-and-context.html', 'WIP'), ('06', 'Orchestrating agents', 'orchestrating-agents.html', 'WIP'), ('07', 'Measuring what works', 'measuring-what-works.html', 'WIP')]
    out = ''.join(f'<a class="dk" href="{u}" target="_blank" rel="noopener"><span class="no">{n}<span class="sep"> · </span></span><span class="lt">{t}{" (" + w + ")" if w else ""}</span><span class="ld">{u} <b aria-hidden="true">↗</b></span></a>' for n, t, u, w in items)
...
TOOLBOX = [
 ...
 slide('Start with a task you already do', io_task(), 'Ask the audience to think of a recurring task they know well enough to review. Our review task uses synthetic data. Do not claim it is a customer case study or that a particular product produced it.', '2, 21–23', 'One real task'),
 ...
 slide('Check one important claim all the way back', claim_chain(), 'A citation is a path to evidence, not an accuracy seal. Read enough surrounding material to check scope and date. If the source is missing, inaccessible or does not support the claim, leave the claim unresolved.', '24, 37', 'Research', dark=True),
 slide('For media, decide what must stay exact', panels((ill_explore(), 'Explore a look', 'Generate alternatives for an image or a short scene.'), (ill_template(), 'Keep a template', 'Use fixed layouts for labels, numbers and repeated versions.')), 'Distinguish exploratory generation from a deterministic template. Brand colours and text should be checked in both. Do not promise perfect brand compliance simply because an output was rendered from code.', '29–34', 'Media'),
 ...
 slide('An app draft still needs an owner', app_owner(), 'An app is executable software, even if a nondeveloper created it. Start with a prototype using synthetic data. A working demo does not establish security, correctness or maintainability.', '28', 'Apps'),
 slide('Automate the repeatable part', stations([('Trigger', 'A new report arrives.'), ('Draft', 'Summarise the changes.'), ('Review', 'A person approves the message.')], ['mail', 'sparkle', 'person'], hot=(2,)), 'This is an illustrative workflow, not a connected automation we have deployed. Tools such as Zapier can connect the steps. A fixed rule may be sufficient for routing; use AI where interpreting the content adds value.', '38, 54', 'Automation'),
 slide('There are three ways into your work', cards(('Upload', 'Give the assistant selected files.'), ('Connect', 'Let it reach an approved data source.'), ('Work in the app', 'Use an assistant beside the document.'), icons=['upload', 'link', 'panel']), 'Return to the review report. You can upload a CSV, connect a source or work in the spreadsheet itself. These routes differ in convenience and access. Choose the smallest scope that covers the task.', '47–55 → compressed', 'Access'),
 slide('A connection grants access', access_scope(), 'MCP is one standard through which tools and data can be exposed to assistants. It does not itself grant permission to use every connected system. The audience only needs the access decision here; protocol details belong later.', '48–49', 'Access', dark=True),
 slide('Check your existing tools first', cards(('Microsoft 365', 'Look for approved assistance inside your Office workflow.'), ('Google Workspace', 'Look for approved assistance in the apps you already use.'), icons=['suite', 'panel']), 'Examples include Microsoft 365 Copilot and Gemini in Workspace. Availability varies by licence and administrator settings. Verify the actual account before purchasing another subscription. Do not turn this into a price comparison.', '50–55', 'Access'),
 slide('Budget for attempts and review', '<div class="v-legend"><span>What you pay for</span><span>What to check</span></div>' + rows(('card', 'Subscription', '<span class="kk">check</span> Usage limits and required features'), ('coins', 'Credits', '<span class="kk">check</span> How many attempts one usable result takes'), ('team', 'Team access', '<span class="kk">check</span> Seats, admin controls and data settings'), ('clock', 'Your time', '<span class="kk">check</span> Review, corrections and rework'), cls='grid2 budget'), 'Remove the promise that a fixed monthly price unlocks an equivalent toolbox everywhere. Ask the supplier about the applicable plan. Comparing the cost per usable result is more informative than comparing a headline fee.', '4, 18–19, 39', 'Cost'),
 slide('A shop agent could act—and lose money', story([('What happened', 'Project Vend found suppliers and adjusted its stock, but also sold items below cost.'), ('What to learn', 'Check the business result, not just whether the agent completed its steps.')], ill_vend()), 'Historical example: Anthropic and Andon Labs, first Project Vend report, June 2025. It also invented payment details. Avoid calling it literal bankruptcy or claiming the experiment represents all agent deployments.', '42', 'Two useful stories', dark=True, foot='<a href="https://www.anthropic.com/research/project-vend-1" target="_blank" rel="noopener">Source: Anthropic &amp; Andon Labs · June 2025</a>'),
 slide('Research ideas still need experiments', story([('What happened', 'Google’s AI co-scientist proposed hypotheses that researchers assessed and tested.'), ('What to learn', 'Use AI to widen the search. Keep the evidence standard for the conclusion.')], ill_funnel()), 'Historical example: Google Research, February 2025. The system helped generate and refine hypotheses; laboratory work remained part of validation. Do not equate a quickly proposed hypothesis with replacing years of experiments.', '43', 'Two useful stories', foot='<a href="https://research.google/blog/accelerating-scientific-breakthroughs-with-an-ai-co-scientist/" target="_blank" rel="noopener">Source: Google Research · February 2025</a>'),
 slide('Some tasks are poor starting points', matrix(), 'This is a selection rule for a first trial. Choose a familiar draft task with a human checkpoint. A specialised production system may address these risks, but that is a different adoption decision from trying a general assistant.', '25, 39, 45', 'Choose a first task'),
 slide('Try one familiar task this week', stations([('Choose', 'A small job you can review.'), ('Brief', 'Give sources, an example and a check.'), ('Compare', 'Was the result useful after corrections?')], ['target', 'brief', 'scale']), 'Invite the audience to choose the task they considered at the beginning. Measure total effort, including review. Stop or change tools if repeated corrections erase the time saved. No need to subscribe to several products.', '56–57 → one close', 'Your next step', dark=True, cls='beat'),
 slide('Keep the guide; skip the memorisation', linkcards(('Tool selection handout', 'reviews/first-three-v2/TOOLBOX-HANDOUT.md', 'book', 'Handout'), ('Example report', 'examples/v2/review-report.html', 'report', 'Artifact'), ('Next: use an agent on a bounded task', 'agentic-ai-v2.html', 'agent', 'Talk 02')), 'Pause for questions here. The following slides are a short reference appendix, not another teaching chapter. The expanded handout preserves breadth without filling the talk with product tables.', '40, 55–57', 'Questions'),
 slide('Reference · document tools', links(('Claude', 'https://claude.com/product/overview'), ('ChatGPT', 'https://chatgpt.com/overview/'), ('Gemini', 'https://gemini.google/overview/'), ('Gemini Notebook', 'https://notebook.google/')), 'Product links are examples, not rankings. Consult the linked pages and your administrator for current features, availability and data settings.', '58 → curated', 'Reference', cls='ref'),
 slide('Reference · create and connect', links(('Gamma', 'https://gamma.app/'), ('Canva', 'https://www.canva.com/canva-ai/'), ('ElevenLabs', 'https://elevenlabs.io/text-to-speech'), ('Lovable', 'https://lovable.dev/'), ('Zapier', 'https://zapier.com/')), 'The handout adds selection questions. It deliberately does not reproduce a price list that will age separately from the vendor page.', '59 → curated', 'Reference', cls='ref'),
 slide('Reference · evidence and access', links(('Project Vend', 'https://www.anthropic.com/research/project-vend-1'), ('AI co-scientist', 'https://research.google/blog/accelerating-scientific-breakthroughs-with-an-ai-co-scientist/'), ('Microsoft 365 Copilot', 'https://www.microsoft.com/en-us/microsoft-365-copilot/business'), ('MCP introduction', 'https://modelcontextprotocol.io/docs/getting-started/intro')), 'These sources support the historical stories and access examples. The editorial source log distinguishes facts, examples and recommendations.', '58–59 → curated', 'Reference', cls='ref'),
]
...
INTRO = [
 ...
 slide('One failing test, one bounded task', failing_test(), 'These are the checked-in fixture values. The 21% rate is a supplied teaching input, not advice about tax law. A passing result for this one input is only the first check.', '2, 13', 'The task'),
 slide('Set the boundaries before you start', boundaries(), 'Show the actual workspace and permission settings. Keep approval prompts for anything outside the task. Do not use bypass mode for presentation convenience. A separate folder is convenient scoping; it is not a security sandbox.', '33–34 moved before demo', 'Before the demo', dark=True),
 slide('Give it the outcome and the checks', window('prompt', ''.join(f'<div class="ln"><span class="no">{i}</span><span class="tx">{esc(t)}</span></div>' for i, t in enumerate(['Fix the failing invoice tests. Explain the cause.', 'Apply the supplied rate; round once, half up.', 'Preserve the tests. Work only in this demo folder.', 'Do not install or publish anything.', 'Run python3 -m unittest -v.', 'Show the result and the diff.'], 1)), 'paper', 'v-brief prompt'), 'Use the full prompt in examples/v2/invoice/README.md. Before the talk, copy before/ to a disposable directory. Start the agent here only after showing boundaries. Approve necessary actions deliberately. If a live run is unavailable, use the prepared checkpoints.', '4, 40 → concrete prompt', 'Start the demo', foot='<a href="examples/v2/invoice/README.md" target="_blank" rel="noopener">Demo setup and fallback</a>'),
 slide('We will inspect four checkpoints', stations([('Failure', 'What broke?'), ('Evidence', 'What did it read?'), ('Change', 'What did it edit?'), ('Check', 'What passed?')], ['warn', 'search', 'pencil', 'tick'], ok=(3,), label='Four checkpoints: failure, evidence, change, check'), 'These are teaching checkpoints, not a promise that every agent takes the same path. If it finishes early, inspect the completed trace. If it stalls, switch to the prepared fixtures and say so.', '4, 12, 15, 30, 33 → coordinated callbacks', 'The route'),
 slide('The model <span class="o">chooses</span>; the harness <span class="o">executes</span>', model_harness(), 'Agent is the combined system when it iterates through a task. The model does not directly execute Python; the harness runs a tool. Harness is a useful term because it explains why the same model behaves differently in different products.', '6, 10', 'Two parts'),
 slide('The agent repeats that exchange', video('agent-loop-dark', 'Choose an action → run the tool → read the result → continue or stop', steps=['Choose an action', 'Run the tool', 'Read the result', 'Continue or stop']), 'Checkpoint: show a tool call from the live run, if available. Use the animation to connect the three parts; do not narrate a fictitious live result. Our failure output is the next slide.', '12', 'The loop', dark=True),
 slide('Checkpoint 1 · the failure is evidence', checkpoint1(), 'Prepared fallback, validated from before/. The actual output also contains test names and tracebacks; this is an excerpt. Ask what the agent should read next. If live output differs, explain the difference rather than claiming it matches.', '2, 13 → checked fallback', 'Inspect the run', foot='Prepared fixture result · not an agent trace', head=mini(1)),
 slide('A tool call produces something inspectable', tool_call(), 'Show the file-read entry in the live trace or open before/invoice.py. Tool schemas and message formats vary across providers. The key is that the result—not the model’s claim that it read something—is the evidence.', '11–13', 'Tools'),
 slide('Checkpoint 2 · the code explains the failure', code_explains(), 'The excerpt omits type annotations and the docstring to keep the slide readable. The full source is in before/invoice.py. The passing zero-rate case is consistent with this bug; the failing nonzero cases distinguish it.', '9, 11 → same task', 'Inspect the run', head=mini(2)),
 slide('Context is what the model can use now', context_now(), 'Context is broader than the latest message. A product may retrieve or summarise previous material. Avoid saying every request literally includes every previous token or that the product has no persistent storage.', '7–11, 28 → compressed', 'Context'),
 slide('More information is not always more help', more_info(), 'Do not teach a universal 40% or 50% threshold. The point at which performance suffers depends on the model, task and content. Missing a necessary file can be as harmful as loading too much irrelevant material.', '29–30', 'Context'),
 slide('When a session loses the thread', handoff(), 'Explain compaction only as a summary that can omit details. Caching affects reused computation and billing; it does not restore omitted facts. Subagents are an optional advanced technique covered in later talks.', '31–32, 40', 'Context'),
 slide('Thinking text is not verification', thinking(), 'Reasoning can help a model tackle a task, but displayed summaries are not a faithful record of every internal cause. Keep the distinction practical: review evidence instead of accepting a confident explanation.', '14–15', 'Reasoning', dark=True),
 slide('Checkpoint 3 · inspect the change', diff_view(), 'This slide isolates the conceptual change. The actual reference implementation names total only in after/. Check the real diff and confirm the rate comes from the argument. It must not simply return 121 or weaken the tests.', '13 → patch review', 'Inspect the run', head=mini(3)),
 slide('Checkpoint 4 · test more than the hook', test_cards(), 'Prepared result: python3 -m unittest test_invoice -v in after/. These cases verify the stated fixture behaviour, not all production financial requirements. The export tests in talk 03 are separate.', '13, 40 → independent checks', 'Inspect the run', foot='Prepared fixture result · three invoice tests', head=mini(4)),
 slide('Passing tests are part of the review', review_list(), 'Inspect the actual output from the live agent if present. The agent may report success while skipping a command or changing tests. The human reviewer decides whether the evidence is sufficient.', '33, 40–41 → stronger finish', 'Review'),
 slide('If it goes wrong, interrupt early', interrupt(), 'If changes are wrong, inspect a diff and use an understood checkpoint or restore a disposable copy. Do not suggest destructive reset commands that would remove unrelated work. A second identical retry without new evidence is often unhelpful.', 'new failure recovery', 'Recovery', dark=True),
 slide('Rules can survive a new session', rules_file(), 'Use the instruction file your harness reads: CLAUDE.md for Claude Code, AGENTS.md where supported. These are textual instructions, not an access-control boundary. Product memory may also retain notes; inspect what is actually loaded.', '35–36', 'Project knowledge'),
 slide('Put recurring procedures in a skill', panels((ill_rules(False), 'Project rules', 'Short facts and boundaries needed across tasks.'), (ill_rules(True), 'Skill', 'A reusable procedure loaded for the relevant task.')), 'A release checklist is a better skill example than another agent glossary. Describe a skill as instructions and optional supporting files. Exact loading and invocation are harness-specific. Plugin packaging belongs in a later talk.', '37', 'Project knowledge'),
 slide('Tools can reach outside the repo', reach(), 'MCP standardises part of the connection between assistants and external tools/data. It does not eliminate authentication or make arbitrary access safe. Our demo has no reason to connect to another system.', '38–39', 'Connected tools'),
 slide('Start with one approved agent', one_agent(), 'This replaces the landscape tour. Model selection can wait until the audience can identify a task and evaluate its result. Tool availability and account features change; point to product documentation for setup.', '17–27 → one practical choice', 'Getting started'),
 slide('Your first session, in five steps', stations([('Describe', 'Name the outcome.'), ('Provide', 'Give relevant context.'), ('Limit', 'Set access and scope.'), ('Inspect', 'Read the change.'), ('Verify', 'Run the checks.')], ['target', 'stack', 'lock', 'search', 'tick'], ok=(4,)), 'Return to the opening task and ask the audience to explain each step. The five-step routine is the takeaway. Do not add another glossary or product comparison after it.', '40–41 → one close', 'Your next step', dark=True, cls='beat'),
 slide('Try the fixture, then a task of your own', linkcards(('Demo files, prompt and fallback', 'examples/v2/invoice/README.md', 'codefile', 'Practice'), ('Next: engineer a repeatable workflow', 'agentic-engineering-v2.html', 'loop', 'Talk 03'), ('Choose a tool for a different kind of work', 'ai-toolbox-v2.html', 'actions', 'Talk 01')), 'Questions and optional practice. The fixture is small enough to inspect fully. For a real task, choose work you already understand and can verify.', '41 → practice', 'Questions'),
 slide('Reference · operating an agent', links(('Claude Code: how it works', 'https://code.claude.com/docs/en/how-claude-code-works'), ('Claude Code: permissions', 'https://code.claude.com/docs/en/permissions'), ('Claude Code: project memory', 'https://code.claude.com/docs/en/memory'), ('Claude Code: skills', 'https://code.claude.com/docs/en/skills')), 'These are provider-specific references, not a claim that every harness behaves identically. Consult current documentation for exact controls.', '42–43 → curated', 'Reference', cls='ref'),
 slide('Reference · context and evidence', links(('Effective context engineering', 'https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents'), ('Reasoning reports and their limits', 'https://www.anthropic.com/research/reasoning-models-dont-say-think'), ('MCP introduction', 'https://modelcontextprotocol.io/docs/getting-started/intro'), ('Building effective agents', 'https://www.anthropic.com/engineering/building-effective-agents')), 'The sources support the conceptual explanation. The fixture and prepared test results are local evidence, separate from provider claims.', '42–43 → curated', 'Reference', cls='ref'),
]
...
ENGINEERING = [
 ...
 slide('A passing demo is only the beginning', demo_vs_repeat(), 'The goal is not maximum autonomy. It is work that can be accepted with evidence and recovered when it fails. Distinguish repeatable workflows from one successful transcript.', '2, 23–25 → new framing', 'The problem'),
 slide('Keep one example throughout', csv_hero(), 'This is an excerpt from the reference CSV. The complete file also exercises a newline in a customer name. Our implementation assumes validated rows; production validation and spreadsheet formula handling are separate requirements.', 'new running example', 'The task', foot='<a href="examples/v2/invoices.csv" download>Download the reference CSV</a>'),
 slide('Recap: the system you are engineering', cycle([('Context', 'Task, rules and evidence.'), ('Model', 'Proposes the next step.'), ('Tools', 'Perform permitted actions.'), ('Result', 'Changes what the system knows.')], ['stack', 'sparkle', 'gear', 'report'], 'A loop: context feeds the model, the model proposes a step, tools act, and the result changes the context.'), 'Compress the original prediction, thinking, tools and statelessness chapters to this recap. We can change what information the system receives, what actions it may take and how results are checked.', '3–21 → recap', 'The machine'),
 slide('Problem 1 · “Add export” leaves too much open', unspecified(), 'Ask the audience what they would need to review an export. The missing information is a contract problem before it is a model problem. Do not respond to ambiguity by increasing reasoning effort.', '25 → expanded contract', 'Define the work', dark=True, layout='problem', pnum=1, plead='<span class="kk">Consequence</span>A plausible implementation may still miss the requirement.'),
 slide('Write the acceptance contract first', contract(), 'These are the requirements implemented by the checked-in reference fixture. For the talk, the set is deliberately small and testable. Label omitted production requirements rather than implying this is a complete billing system.', '23–25 → explicit contract', 'Define the work'),
 slide('Decide how much structure the task needs', structure(), 'This is a design choice, not a maturity ladder. A workflow may contain agent-driven stages. Use the simplest approach that can meet the contract and explain its result.', '20, 48', 'Choose the process'),
 slide('Make the stages and ownership visible', stations([('Inspect', 'Locate code and tests.'), ('Plan', 'Resolve the contract.'), ('Build', 'Make the bounded change.'), ('Verify', 'Run independent checks.'), ('Review', 'Accept or send back.')], ['search', 'list', 'code', 'tick', 'person'], hot=(4,), back=(4, 2, 'send back')), 'This is the workflow for our example. Keep a named owner for integration and acceptance. The next slides show how evidence and recovery change what happens at the gates.', '46–48 → core workflow', 'The workflow'),
 slide('Problem 2 · a green command can mislead', exit_zero(), 'An exit code is an observation. It does not prove the tests cover the contract. The original deck said a goal prompt could not be gamed; replace that guarantee with independent checks and review.', '23–25', 'Check the evidence', dark=True, layout='problem', pnum=2, plead='It may have run the wrong tests, skipped a case or checked a weaker requirement.'),
 slide('Give every claim a check', claim_check(), 'The reference test includes a comma, embedded quotes and a newline. Parsing with the CSV reader tests serialization rather than eyeballing a text file. Formula-like input remains outside this fixture’s contract.', '23–25 → example verification', 'Check the evidence'),
 slide('Keep the calculation outside the guesswork', calc_code(), 'Use deterministic code for a specified calculation. Let the agent inspect and change the implementation, but verify against the explicit rounding rule. This is the existing calculation from the reference fix, reused by the exporter.', '15, 25 → deterministic tool example', 'Check the evidence'),
 slide('Inspect both tests and the artifact', tests_and_artifact(), 'Prepared reference result from after/: three invoice tests and two export tests. The CSV is generated with the same implementation and parsed in validation. This is not a benchmark or a captured agent run.', '23–25 → checked evidence', 'Check the evidence', foot='Prepared reference result · five tests'),
 slide('Keep acceptance separate from implementation', two_actor(), 'Another model can help review but can share the same blind spots. Protect important acceptance checks outside the implementation’s control when warranted. Human review still owns the decision for this workflow.', '24–25, 42 → review ownership', 'Check the evidence'),
 slide('Problem 3 · the useful facts get buried', buried(), 'Preserve the original context thread without treating attention as a simple fixed budget divided equally between tokens. More irrelevant material can harm retrieval and distract the model; exact behaviour varies.', '26–34 → context diagnosis', 'Manage context', dark=True, layout='problem', pnum=3, plead='Exploration accumulates logs, dead ends and outdated assumptions.'),
 slide('Give each stage the context it needs', stage_context(), 'The table is our proposed design, not an automatic feature of every harness. Fresh stages must receive enough context to preserve decisions. A short handoff is only useful if it contains the necessary facts.', '18, 27, 36, 46', 'Manage context'),
 slide('Store decisions before restarting', handoff_note(), 'This is a handoff example for the reference fixture. Keep file references and unresolved scope, not pages of tool output. A new session should verify the handoff against the files rather than trusting it blindly.', '34, 50–51 → useful handoff', 'Manage context'),
 slide('Caching and compaction solve different problems', cache_vs_compaction(), 'Billing rules, cache lifetimes and treatment of reasoning vary by provider and model. Avoid the original blanket claims about byte-identical requests, perpetual warm caches or thinking always being retained/stripped. Detailed mechanics belong in talk 04.', '31–34 → two distinctions', 'Manage context'),
 slide('Problem 4 · retries consume the budget', budget(), 'Do not let a retry loop keep editing indefinitely. Distinguish transient tool failure, misunderstanding and an unmet requirement. Each calls for a different response.', '22, 24 → bounded retries', 'Recover', dark=True, layout='problem', pnum=4, plead='A failed attempt should change the next decision.'),
 slide('Set a retry policy before the run', retry_tree(), 'This is a policy sketch, not a provider command. Choose numeric limits for the real environment. A stop should preserve useful evidence and never imply the task succeeded.', '22–25 → stop policy', 'Recover'),
 slide('Recovery starts with an inspectable boundary', recovery(), 'A branch or worktree isolates edits, not credentials or network access. Use environment isolation separately when needed. Never blindly reset a shared dirty workspace; preserve unrelated changes.', '24, 44 → recovery boundary', 'Recover'),
 slide('Problem 5 · cheap tokens can buy costly work', fraction(), 'Do not claim stronger or smaller models are always cheaper. Compare the total process on representative tasks and hold acceptance criteria constant. Subscription limits and API charges are different meters.', '29–31, 38–45 → cost per accepted task', 'Choose the model', dark=True, layout='problem', pnum=5, plead=''),
 slide('Compare accepted work, not headline rates', accepted_chart(), 'Hypothetical arithmetic, not measured model performance. Both spend one dollar per accepted task, but B accepts more of the batch and uses less review time. Track failures, latency and human effort separately rather than collapsing them into one misleading number.', '30, 38 → labelled comparison', 'Choose the model', foot='Illustrative numbers · not vendor measurements', tsize=80),
 slide('Change one dial and measure again', stations([('Baseline', 'A few representative tasks and fixed checks.'), ('Trial', 'Change model or reasoning effort.'), ('Compare', 'Acceptance, cost, time and review burden.')], ['flag', 'dial', 'scale'], hot=(1,)), 'The set should include common tasks and failure-prone cases. Repeat enough to see variability. Re-evaluate after meaningful model, prompt or tool changes; do not teach a permanent ranking of named tiers.', '37–45 → evaluation method', 'Choose the model'),
 slide('Problem 6 · one agent carries too much', video('subagents-dark', 'Separate investigation → evidence back · token counts in the animation are illustrative.', cls='stacked'), 'Delegation may help when investigation is separable or parallelism saves time. It adds coordination, context handoff and cost. It is not required for this small export fixture. The animation’s 80k/200 token counts are illustrative, not measurements of this example.', '35–36', 'Delegate selectively', dark=True, layout='problem', pnum=6, plead=''),
 slide('Delegate an outcome with a boundary', delegate(), 'This is a reviewer brief for the same example, not an instruction to spawn agents during this revision. Findings must be checked before acting. A short conclusion without evidence is not enough.', '42–44 → bounded delegation', 'Delegate selectively'),
 slide('Parallel work still needs integration', fan_in(), 'Independent read-only reviews are easier to combine than concurrent edits to the same files. Use worktrees for independent edits when appropriate; a shared test run on the integrated result still matters.', '44–48 → integration', 'Delegate selectively'),
 slide('Problem 7 · useful lessons disappear', rows(('pin', 'Keep a rule', 'A short constraint needed across tasks.'), ('checklist', 'Keep a procedure', 'A skill for a recurring workflow.'), ('link', 'Keep evidence', 'A decision note linked to the relevant code or test.'), cls='keepers'), 'Avoid turning every solved task into permanent context. Retain what will change future behaviour and periodically remove stale instructions. Plugin distribution is a later concern.', '49–58 → durable knowledge', 'Make it repeatable', dark=True, layout='problem', pnum=7, plead=''),
 slide('Write one small procedure people can inspect', procedure(), 'This is an example skill body rather than a full provider-specific package. Keep the procedure readable and useful before distributing it. It does not itself enforce permissions or guarantee a complete review.', '51–58 → one reusable procedure', 'Make it repeatable'),
 slide('The complete workflow has a way back', way_back(), 'Trace the invoice export once through the workflow. Return to the relevant stage when new evidence invalidates a decision. On budget exhaustion, preserve a partial result and mark it incomplete. Success is reviewed evidence against a contract.', '46–48, 59 → synthesis', 'Bring it together', track=PROBLEMS + 1),
 slide('Improve one workflow this week', rows(('doc', 'Write the contract', 'What must be true when the task is done?'), ('tick', 'Add the evidence', 'Which check can establish it?'), ('exit', 'Add the exit', 'How will you stop and recover when it fails?'), cls='closing'), 'This is the single closing action. Encourage a small change to an existing workflow before adding fleets of agents or a plugin marketplace.', '59 → concrete close', 'Your next step', dark=True, cls='beat'),
 slide('Go deeper where your work needs it', deck_cards(), 'Questions here. The later decks are existing editions, not revised as part of this work. They retain their own date stamps and some are work in progress. Do not imply their product claims were revalidated in this revision.', 'new series handoff', 'Questions'),
 slide('Reference · workflows and evaluation', links(('Building effective agents', 'https://www.anthropic.com/engineering/building-effective-agents'), ('Demystifying evals for AI agents', 'https://www.anthropic.com/engineering/demystifying-evals-for-ai-agents'), ('Effective context engineering', 'https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents'), ('Fixture, tests and reference implementation', 'examples/v2/invoice/README.md')), 'These sources support the design distinctions. The workflow, retry table and illustrative comparison are teaching designs, not measured production outcomes. Detailed evidence and validation are recorded with this edition.', '60 → curated', 'Reference', cls='ref'),
]
...
.v-ta{display:grid;grid-template-columns:1fr 1.2fr;gap:44px;align-items:stretch}
.v-okterm .wbody{font-size:38px;padding:36px 48px 40px}
.v-art{background:var(--s-card-bg);border:1.5px solid var(--s-card-border);border-radius:20px;padding:28px 36px}
.v-art .ah{display:flex;align-items:center;gap:14px;font:500 26px var(--font-mono);color:var(--s-muted);margin-bottom:12px}
.v-art .ah .ic{width:44px;height:44px}
.v-art table{width:100%;border-collapse:collapse;font:500 30px var(--font-mono);color:var(--s-text)}
.v-art th{text-align:left;font:600 22px var(--font-mono);letter-spacing:.06em;color:var(--s-muted);padding:12px 12px;border-bottom:3px solid var(--coral)}
.v-art th b{display:block;color:var(--s-accent);text-transform:uppercase;letter-spacing:.12em;font-size:20px}
.v-art td{padding:16px 12px;border-bottom:1px solid var(--s-line)}
.v-art td:nth-child(2){color:var(--coral-ink)}
.v-art .cap{font:600 22px var(--font-mono);letter-spacing:.12em;text-transform:uppercase;color:var(--s-accent);margin-top:14px}
